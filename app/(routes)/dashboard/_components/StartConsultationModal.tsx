...
import { Sparkles, X, Info, ArrowRight, Loader2 } from "lucide-react";
...
export default function StartConsultationModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (selectedAgent) {
      await handleStartConsultation(selectedAgent);
    }
  };
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-455 dark:text-slate-400 -mt-0.5">
                Intake with {selectedAgent.specialist}
              </p>
            </div>
          </div>
          <button
              ...
              <div>
                <h4 className="text-xs font-bold text-slate-850 dark:text-slate-100 flex items-center gap-1.5 flex-wrap">
                  ...
                  {selectedAgent.subscriptionRequired ? (
                    <span className="inline-flex items-center gap-0.5 px-1.5 py-0.2 rounded-full text-[8px] font-bold bg-amber-500/10 text-amber-700 dark:text-amber-450 border border-amber-555/15">Pro</span>
                  ) : (
